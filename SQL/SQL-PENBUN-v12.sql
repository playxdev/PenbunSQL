/* =====================================================================================
   SQL-PENBUN-v12.sql   —   STANDALONE FULL BUILD
   -------------------------------------------------------------------------------------
   Generated : 2026-09-13
   Scope     : DROP & CREATE ทั้งฐานข้อมูล  38 ตาราง  ไม่มี ALTER แม้แต่บรรทัดเดียว
   Replaces  : SQL-PENBUN-v11.sql  (ห้ามรันร่วมกัน)

   !!! Full Rebuild : SECTION 1 ลบทุก object ในฐาน  ฐานที่มีข้อมูลจริงต้องสำรองก่อนเสมอ !!!

   !!! คำเตือน : SECTION 1 ลบข้อมูลทั้งหมด  สำรองก่อนรันเสมอ !!!

   ---------------------------------------------------------------------------------
   สิ่งที่ v12 เพิ่มจาก v11  —  RBAC (Roadmap ข้อ 1 · Authentication Spec M002)
   ---------------------------------------------------------------------------------
   ตั้งแต่ v4 ตัวที่ตัดสินสิทธิ์คือ tb_users.user_level คอลัมน์เดียว สองค่า
   ADMIN / USER  ตั้งแต่ 8 ก.ย. 2026 สองค่านี้ถูกใช้จริงจัง : อ่านข้อมูลพื้นฐานได้
   ทุกคนที่ login  เขียนได้เฉพาะ ADMIN  บังคับที่ crud.Resource.RequireLevelWrite
   และ Resource.Validate ทำให้ process ไม่ start ถ้า descriptor ไหนลืมประกาศ

   สิ่งที่สองค่าพูดไม่ได้คือทุกอย่างที่แคบกว่าทั้งตาราง — "คนนี้โพสต์ใบรับสินค้าได้
   แต่แก้ลูกค้าไม่ได้" · "สาขานี้เห็นเฉพาะคลังตัวเอง" · "บทบาทนี้เห็นจำนวน
   แต่ไม่เห็นต้นทุน"  สเปก M002 หน้า 23-36 ออกแบบคำตอบไว้แล้วตั้งแต่ต้น
   v12 คือการเอาโมเดลนั้นลงฐาน

   [A] tb_role              บทบาท  role_code ห้ามซ้ำในแถวที่ยังไม่ถูกลบ
       is_system = 1 คือบทบาทที่ระบบพึ่งพา (ADMIN / USER)
       สเปกหน้า 24 ระบุว่า Role ไม่มีหน้าจอ  เพิ่ม/ลบ/แก้ทำที่ฐานโดย database admin
       เท่านั้น  v12 จึงไม่มี Procedure สำหรับเขียนตารางนี้โดยตั้งใจ

   [B] tb_user_role         ผู้ใช้ M..N บทบาท ตาม ER หน้า 33
       หน้าจอ P0002 ของสเปกมีช่อง Role ช่องเดียว ซึ่งขัดกับ ER ในเอกสารเดียวกัน
       v12 เลือกตาม ER เพราะตารางรองรับหนึ่งบทบาทได้อยู่แล้ว แต่ ER รองรับ
       หลายบทบาทได้โดยไม่ต้องแก้โครงทีหลัง  คู่ (user, role) ห้ามซ้ำ

   [C] tb_privilege_group   กลุ่มสิทธิ์ = โมดูล  SYSTEM / MASTER / DOCUMENT / STOCK

   [D] tb_privilege         หนึ่งแถว = บทบาท x resource x สี่การกระทำ
       can_view · can_insert · can_update · can_delete  ตรงกับช่องติ๊กสี่ช่อง
       บนหน้าจอ M002_P0004  ส่วน resource_code คือชื่อ descriptor ฝั่ง PenbunAPI
       (crud.Resource.Name) จึงผูกกับเส้นทางที่ mount จริง ไม่ใช่รายการเมนูที่
       เขียนซ้ำไว้ต่างหาก  หนึ่งบทบาทมีได้แถวเดียวต่อ resource

   [E] vw_role · vw_privilege · vw_user_privilege
       vw_user_privilege คือสิทธิ์ผลลัพธ์ของผู้ใช้หนึ่งคน อ่านครั้งเดียวจบ
       ผู้ใช้ถือได้หลายบทบาท สิทธิ์จึงรวมแบบ union — บทบาทใดให้ผ่านก็ผ่าน
       GET /auth/me ส่งรายการนี้กลับไปให้หน้าจอได้ตรง ๆ

   ความละเอียดที่ได้คือ รายโมดูล x resource x CRUD  ไม่ใช่รายฟิลด์ และไม่ใช่รายสาขา
   สเปก M002 ไม่ได้ออกแบบสองอย่างหลังไว้  ใครต้องการต้องออกแบบเพิ่ม ไม่ใช่ขยายของเดิม

   user_level ยังอยู่ครบใน v12 และยังเป็นตัวที่ PenbunAPI ใช้จริง
   SEED 11.16 ลอกกฎที่ API บังคับอยู่วันนี้ลงตาราง privilege ทั้งดุ้น ติดตั้ง v12
   แล้วสิทธิ์ของทุกคนจึงเท่าเดิมทุกประการ  คอลัมน์ user_level ตัดทิ้งได้เมื่อ API
   ย้ายไปอ่าน vw_user_privilege แล้วเท่านั้น — ไม่ใช่ในไฟล์นี้

   History Log (Roadmap ข้อ 2) ยังไม่ได้ทำ  สเปก M002 บังคับว่าทุก insert/update/delete
   บนผู้ใช้และสิทธิ์ต้องเขียนประวัติ และหน้าจอต้องแสดงย้อนหลัง 5 รายการ
   v12 จึงยังไม่ครบสเปก M002 ทั้งโมดูล — ครบเฉพาะส่วนสิทธิ์

   ---------------------------------------------------------------------------------
   สิ่งที่ v11 เพิ่มจาก v10  —  Read Model ของผู้ใช้งาน
   ---------------------------------------------------------------------------------
   tb_users มีมาตั้งแต่ v4 แต่ไม่เคยมี View  PenbunAPI จึงอ่านตารางนี้ได้ทางเดียว
   คือ SELECT ตรงในแพ็กเกจ auth ตอน login  หน้าจอ "ผู้ใช้และสิทธิ์" ฝั่ง PenbunWeb
   เลยเป็นข้อมูลปลอมมาตั้งแต่ต้น เพราะไม่มีอะไรให้อ่าน

   [A] vw_users  —  ผู้ใช้งานที่ยังไม่ถูกลบ พร้อมคลังประจำตัวที่ JOIN มาให้แล้ว
       ไม่คืน user_password และไม่คืน counting_password_fail
       View นี้เป็น Read Model ล้วน ๆ  รหัสผ่านไม่ควรออกจากตารางไปไหนทั้งนั้น
       แม้แต่ในรูป hash  ส่วนจำนวนครั้งที่กรอกผิดเป็นสถานะภายในของ auth
       ที่ status_user_locked สรุปให้แล้ว

   ไม่มีตารางใหม่ ไม่มี Procedure หรือ Trigger ที่เปลี่ยน
   จำนวน View ใน SECTION 12 เพิ่มจาก 32 เป็น 33  บรรทัดอื่นเท่า v10 ทุกบรรทัด

   ---------------------------------------------------------------------------------
   สิ่งที่ v10 เพิ่มจาก v9  —  ที่อยู่ชุดเดียวกันทั้งสี่ตาราง
   ---------------------------------------------------------------------------------
   สี่ตารางที่เก็บที่อยู่เคยไม่เหมือนกันสักตาราง  tb_customer และ tb_vendor มีครบห้า
   คอลัมน์  tb_company มีครบแต่ View ไม่คืนสองคอลัมน์  ส่วน tb_warehouse มีแค่
   address กับ province  ผลคือหน้าจอเดียวกันกรอกที่อยู่ได้ไม่เท่ากัน และตัวเลือก
   จังหวัด → อำเภอ → ตำบล → รหัสไปรษณีย์ ฝั่ง PenbunWeb ทำได้แค่สองหน้าจากสี่

   [A] tb_warehouse  เพิ่ม sub_district · district · zip_code
       nvarchar(100) / nvarchar(100) / nvarchar(20)  NULL ทั้งสามคอลัมน์
       ชนิดและความยาวตรงกับ tb_customer · tb_vendor · tb_company ทุกตัวอักษร
       คลังไม่ได้ลอยอยู่กลางอากาศ  ใบส่งของและใบรับคืนอ้างที่อยู่คลังเหมือนกับ
       อ้างที่อยู่ลูกค้า  การเก็บแค่จังหวัดจึงเป็นช่องว่าง ไม่ใช่การตัดสินใจ

   [B] vw_warehouse  คืนสามคอลัมน์ใหม่
   [C] vw_company    คืน sub_district กับ district ที่ตารางมีอยู่แล้วตั้งแต่ v5
       แต่ View ไม่เคย SELECT  PenbunAPI จึงมองไม่เห็นและเขียนกลับไม่ได้

   ไม่มีตารางใหม่ ไม่มีตารางที่หายไป ไม่มี Procedure หรือ Trigger ที่เปลี่ยน
   จำนวน object ใน SECTION 12 จึงเท่า v9 ทุกบรรทัด

   ---------------------------------------------------------------------------------
   สิ่งที่ v9 เพิ่มจาก v8  —  แม็ปส่วนลด (Roadmap v9 ข้อ 4 · §13.6 ของ v8)
   ---------------------------------------------------------------------------------
   v8 ตอบไม่ได้ว่าร้านหนึ่งจ่ายเท่าไหร่สำหรับหนังสือเล่มหนึ่ง  tb_discount เป็นแคมเปญ
   แบน ๆ ไม่มีมิติลูกค้าและสินค้า  ส่วน tb_customer.discount_group เป็นข้อความอิสระ
   ที่ไม่มีอะไร resolve ให้  tb_order_item.discount_percent จึงเป็นเลขที่คนคีย์พิมพ์เอง

   [A] tb_discount_group (DCG)  —  กลุ่มส่วนลดลูกค้า  แทน legacy tb_DiscountGroups
       tb_customer.discount_group (nvarchar) กลายเป็น ref_discount_group_auto (FK จริง)

   [B] tb_price_rule (PRL)  —  กฎส่วนลดสี่มิติในตารางเดียว
       legacy กระจายไว้สามตาราง (กลุ่ม / on-top รายสาขา / เฉพาะเล่ม) + Amount1-3 บนตัวหนังสือ
       v9 รวมเป็นตารางเดียวแยกด้วย rule_scope  โดยมี CK_tb_price_rule_target บังคับว่า
       ปลายทางต้องตรงกับ scope ที่ประกาศ  เพิ่มมิติใหม่ = เพิ่มค่า enum ไม่ใช่เพิ่มตาราง
         CUSTOMER_SKU · CUSTOMER · ROUTE · GROUP_SKU · GROUP · SKU
       ขั้นบันไดจำนวนเป็น min_qty/max_qty หนึ่งขั้นหนึ่งแถว แทน Amount1/2/3 สามคอลัมน์

   [C] UFN_RESOLVE_DISCOUNT  —  ตัวเดียวที่ตอบว่าร้านนี้จ่ายเท่าไหร่
       is_on_top = 0 แข่งกัน เจาะจงกว่าชนะ (ทับ) :
         CUSTOMER_SKU > CUSTOMER > ROUTE > GROUP_SKU > GROUP > SKU > tb_book default > 0
       is_on_top = 1 บวกทับผู้ชนะ (ซ้อน) = legacy tb_OnTopDiscounts ตรงตัว
       คืน source_scope มาด้วย หน้าจอจึงบอกที่มาของตัวเลขได้ ไม่ใช่ช่องเปล่าที่ไม่มีใครรับรอง
       ***ลำดับความเจาะจงยังรอศูนย์กระจายสินค้ายืนยัน*** เอกสาร legacy ไม่ได้เขียนไว้

   [D] TRIG_BLOCK_OVERLAP_TB_PRICE_RULE  —  กันกฎที่ปลายทางและช่วงวันซ้อนกัน
       CHECK constraint มองได้ทีละแถวจึงกันไม่ได้  ทริกเกอร์ใช้ THROW ไม่ใช่ RAISERROR
       + ROLLBACK เพราะ ROLLBACK ในทริกเกอร์ทำให้ client ได้ error 3609 แทนเลข 50000
       ของเราเอง แล้ว PenbunAPI แปลเป็น INTERNAL แทนที่จะเป็น BUSINESS_RULE

   [E] View ใหม่สองตัว  vw_discount_group · vw_price_rule
       และ vw_customer คืนกลุ่มส่วนลดเป็น Business ID + ชื่อ แทนข้อความอิสระ

   ที่มาและกติกาเต็ม : ../DISCOUNT-MODEL.md  ·  เทสต์ : TEST/TEST-discount-resolve.sql

   ---------------------------------------------------------------------------------
   สิ่งที่ v8 แก้จาก v7  (ยังอยู่ครบใน v9)
   ---------------------------------------------------------------------------------
...
/* v9 สร้างฐานให้ถ้ายังไม่มี — v8 ไม่ได้ทำ ฐานที่เพิ่งลบทิ้งจึงรันแล้วตายที่ USE ทันที
   มีฐานอยู่แล้วบรรทัดนี้ไม่ทำอะไร ข้อมูลเดิมถูกลบด้วย SECTION 1 เหมือนเดิม */
IF DB_ID(N'PENBUN') IS NULL
    CREATE DATABASE [PENBUN];
GO
...
-- v9 : UFN_* เป็น function ไม่ใช่ procedure  sys.procedures จึงมองไม่เห็น
SET @sql = N'';
SELECT @sql = @sql + N'DROP FUNCTION [dbo].[' + name + N'];' + CHAR(10)
  FROM sys.objects WHERE name LIKE 'UFN[_]%' AND type IN ('IF', 'FN', 'TF');
EXEC sp_executesql @sql;
...
/* =====================================================================================
   SECTION 3 : CREATE TABLES  (38 ตาราง เรียงตาม dependency)
   ===================================================================================== */
...
/* ═══════════ Layer 0 : RBAC  (v12 — Authentication Spec M002) ═══════════ */
/****** [dbo].[tb_role]  --  บทบาท (สเปก M002 : ไม่มีหน้าจอ แก้ที่ฐานเท่านั้น) ******/
CREATE TABLE [dbo].[tb_role](
	...
	[is_delete] [bit] NOT NULL,
	[id_status] [nvarchar](20) NOT NULL,
 CONSTRAINT [PK_tb_role] PRIMARY KEY CLUSTERED ([autoID] ASC)
  WITH (PAD_INDEX = OFF, STATISTICS_NORECOMPUTE = OFF, IGNORE_DUP_KEY = OFF,
        ALLOW_ROW_LOCKS = ON, ALLOW_PAGE_LOCKS = ON, OPTIMIZE_FOR_SEQUENTIAL_KEY = OFF) ON [PRIMARY]
) ON [PRIMARY]
GO

/****** [dbo].[tb_user_role]  --  ผู้ใช้ M..N บทบาท (ER หน้า 33 ของสเปก M002) ******/
CREATE TABLE [dbo].[tb_user_role](
	[autoID] [int] IDENTITY(1,1) NOT NULL,
	[prefix] [nvarchar](3) NOT NULL,
	[user_role_id] [nvarchar](50) NULL,
	[ref_user_auto] [int] NOT NULL,
	[ref_role_auto] [int] NOT NULL,
	[update_by] [nvarchar](50) NOT NULL,
	[update_date] [datetime] NOT NULL,
	[is_active] [bit] NOT NULL,
	[is_delete] [bit] NOT NULL,
	[id_status] [nvarchar](20) NOT NULL,
 CONSTRAINT [PK_tb_user_role] PRIMARY KEY CLUSTERED ([autoID] ASC)
  WITH (PAD_INDEX = OFF, STATISTICS_NORECOMPUTE = OFF, IGNORE_DUP_KEY = OFF,
        ALLOW_ROW_LOCKS = ON, ALLOW_PAGE_LOCKS = ON, OPTIMIZE_FOR_SEQUENTIAL_KEY = OFF) ON [PRIMARY]
) ON [PRIMARY]
GO

/****** [dbo].[tb_privilege_group]  --  กลุ่มสิทธิ์ = โมดูล (SYSTEM/MASTER/DOCUMENT/STOCK) ******/
CREATE TABLE [dbo].[tb_privilege_group](
	[autoID] [int] IDENTITY(1,1) NOT NULL,
	[prefix] [nvarchar](3) NOT NULL,
	[privilege_group_id] [nvarchar](50) NULL,
	...
	[is_delete] [bit] NOT NULL,
	[id_status] [nvarchar](20) NOT NULL,
 CONSTRAINT [PK_tb_privilege_group] PRIMARY KEY CLUSTERED ([autoID] ASC)
  WITH (PAD_INDEX = OFF, STATISTICS_NORECOMPUTE = OFF, IGNORE_DUP_KEY = OFF,
        ALLOW_ROW_LOCKS = ON, ALLOW_PAGE_LOCKS = ON, OPTIMIZE_FOR_SEQUENTIAL_KEY = OFF) ON [PRIMARY]
) ON [PRIMARY]
GO

/****** [dbo].[tb_privilege]  --  สิทธิ์หนึ่งแถว = บทบาท x resource x สี่การกระทำ
        ตรงกับหน้าจอ M002_P0004 : เลือก Role + Privilege group + privilege
        แล้วติ๊ก view / insert / update / delete
        resource_code = ชื่อ descriptor ใน PenbunAPI (crud.Resource.Name) ******/
CREATE TABLE [dbo].[tb_privilege](
	[autoID] [int] IDENTITY(1,1) NOT NULL,
	[prefix] [nvarchar](3) NOT NULL,
	[privilege_id] [nvarchar](50) NULL,
	[ref_role_auto] [int] NOT NULL,
	[ref_privilege_group_auto] [int] NOT NULL,
	[resource_code] [nvarchar](50) NOT NULL,
	[can_view] [bit] NOT NULL,
	[can_insert] [bit] NOT NULL,
	...
	[is_delete] [bit] NOT NULL,
	[id_status] [nvarchar](20) NOT NULL,
 CONSTRAINT [PK_tb_privilege] PRIMARY KEY CLUSTERED ([autoID] ASC)
  WITH (PAD_INDEX = OFF, STATISTICS_NORECOMPUTE = OFF, IGNORE_DUP_KEY = OFF,
        ALLOW_ROW_LOCKS = ON, ALLOW_PAGE_LOCKS = ON, OPTIMIZE_FOR_SEQUENTIAL_KEY = OFF) ON [PRIMARY]
) ON [PRIMARY]
GO
...
/****** [dbo].[tb_discount_group]  --  กลุ่มส่วนลดลูกค้า (legacy : tb_DiscountGroups) ******/
CREATE TABLE [dbo].[tb_discount_group](
	[autoID] [int] IDENTITY(1,1) NOT NULL,
	[prefix] [nvarchar](3) NOT NULL,
	[discount_group_id] [nvarchar](50) NULL,
	[group_code] [nvarchar](20) NOT NULL,
	...
	[is_delete] [bit] NOT NULL,
	[id_status] [nvarchar](20) NOT NULL,
 CONSTRAINT [PK_tb_discount_group] PRIMARY KEY CLUSTERED ([autoID] ASC)
  WITH (PAD_INDEX = OFF, STATISTICS_NORECOMPUTE = OFF, IGNORE_DUP_KEY = OFF,
        ALLOW_ROW_LOCKS = ON, ALLOW_PAGE_LOCKS = ON, OPTIMIZE_FOR_SEQUENTIAL_KEY = OFF) ON [PRIMARY]
) ON [PRIMARY]
GO

/* กลุ่มไม่มีคอลัมน์ "ส่วนลดพื้นฐาน" โดยตั้งใจ : ค่านั้นคือ tb_price_rule แถวหนึ่ง
   ที่ rule_scope = 'GROUP'  เก็บไว้ที่เดียวกับส่วนลดชั้นอื่น จึงมีช่วงวันที่และ
   ประวัติการเปลี่ยนเหมือนกัน ไม่ใช่ตัวเลขลอยที่แก้แล้วไม่รู้ว่าใครแก้เมื่อไหร่ */
...
CREATE TABLE [dbo].[tb_customer](
	...
	[ref_discount_group_auto] [int] NULL,
...
/* ═══════════ Layer 10 : Price Rules ═══════════ */
/****** [dbo].[tb_price_rule]  --  กฎส่วนลด 4 มิติ (legacy : 1.8 · 2.5 · OnTop · เจ้าของหนังสือ) ******/
CREATE TABLE [dbo].[tb_price_rule](
	[autoID] [int] IDENTITY(1,1) NOT NULL,
	[prefix] [nvarchar](3) NOT NULL,
	[price_rule_id] [nvarchar](50) NULL,
	[rule_code] [nvarchar](20) NULL,
	[rule_name] [nvarchar](150) NOT NULL,
	[rule_scope] [nvarchar](20) NOT NULL,

	/* ปลายทาง — CHECK CK_tb_price_rule_target บังคับให้ตรงกับ rule_scope */
	[ref_discount_group_auto] [int] NULL,
	[ref_customer_auto] [int] NULL,
	[ref_route_auto] [int] NULL,
	[ref_sku_auto] [int] NULL,

	/* มูลค่า — ใส่ได้อย่างใดอย่างหนึ่ง
	   discount_percent : ส่วนลดเป็น %  ·  net_price : ราคาสุทธิต่อหน่วย (legacy 1.8 ให้มาแบบนี้) */
	[discount_percent] [decimal](5, 2) NULL,
	[net_price] [decimal](18, 4) NULL,

	/* ขั้นบันไดจำนวน — legacy tb_Books.Amount1/2/3 หนึ่งขั้นหนึ่งแถว */
	[min_qty] [int] NULL,
	[max_qty] [int] NULL,

	/* 1 = บวกทับผู้ชนะ (legacy OnTop)  ·  0 = แข่งกัน เจาะจงกว่าชนะ */
	[is_on_top] [bit] NOT NULL,
	/* ตัวตัดสินเมื่อ scope เท่ากันและช่วงวันซ้อนกัน มากชนะ */
	...
	[is_delete] [bit] NOT NULL,
	[id_status] [nvarchar](20) NOT NULL,
 CONSTRAINT [PK_tb_price_rule] PRIMARY KEY CLUSTERED ([autoID] ASC)
  WITH (PAD_INDEX = OFF, STATISTICS_NORECOMPUTE = OFF, IGNORE_DUP_KEY = OFF,
        ALLOW_ROW_LOCKS = ON, ALLOW_PAGE_LOCKS = ON, OPTIMIZE_FOR_SEQUENTIAL_KEY = OFF) ON [PRIMARY]
) ON [PRIMARY]
GO
...
ALTER TABLE [dbo].[tb_role] ADD CONSTRAINT [DF_tb_role_prefix] DEFAULT (N'RLE') FOR [prefix];
ALTER TABLE [dbo].[tb_role] ADD CONSTRAINT [DF_tb_role_update_by] DEFAULT (N'System') FOR [update_by];
ALTER TABLE [dbo].[tb_role] ADD CONSTRAINT [DF_tb_role_update_date] DEFAULT (CONVERT([datetime],(sysdatetimeoffset() AT TIME ZONE 'SE Asia Standard Time'))) FOR [update_date];
ALTER TABLE [dbo].[tb_role] ADD CONSTRAINT [DF_tb_role_is_active] DEFAULT (1) FOR [is_active];
ALTER TABLE [dbo].[tb_role] ADD CONSTRAINT [DF_tb_role_is_delete] DEFAULT (0) FOR [is_delete];
ALTER TABLE [dbo].[tb_role] ADD CONSTRAINT [DF_tb_role_id_status] DEFAULT (N'ACTIVE') FOR [id_status];
ALTER TABLE [dbo].[tb_role] ADD CONSTRAINT [DF_tb_role_is_system] DEFAULT (0) FOR [is_system];
GO

ALTER TABLE [dbo].[tb_user_role] ADD CONSTRAINT [DF_tb_user_role_prefix] DEFAULT (N'URO') FOR [prefix];
ALTER TABLE [dbo].[tb_user_role] ADD CONSTRAINT [DF_tb_user_role_update_by] DEFAULT (N'System') FOR [update_by];
ALTER TABLE [dbo].[tb_user_role] ADD CONSTRAINT [DF_tb_user_role_update_date] DEFAULT (CONVERT([datetime],(sysdatetimeoffset() AT TIME ZONE 'SE Asia Standard Time'))) FOR [update_date];
ALTER TABLE [dbo].[tb_user_role] ADD CONSTRAINT [DF_tb_user_role_is_active] DEFAULT (1) FOR [is_active];
ALTER TABLE [dbo].[tb_user_role] ADD CONSTRAINT [DF_tb_user_role_is_delete] DEFAULT (0) FOR [is_delete];
ALTER TABLE [dbo].[tb_user_role] ADD CONSTRAINT [DF_tb_user_role_id_status] DEFAULT (N'ACTIVE') FOR [id_status];
GO

ALTER TABLE [dbo].[tb_privilege_group] ADD CONSTRAINT [DF_tb_privilege_group_prefix] DEFAULT (N'PVG') FOR [prefix];
ALTER TABLE [dbo].[tb_privilege_group] ADD CONSTRAINT [DF_tb_privilege_group_update_by] DEFAULT (N'System') FOR [update_by];
ALTER TABLE [dbo].[tb_privilege_group] ADD CONSTRAINT [DF_tb_privilege_group_update_date] DEFAULT (CONVERT([datetime],(sysdatetimeoffset() AT TIME ZONE 'SE Asia Standard Time'))) FOR [update_date];
ALTER TABLE [dbo].[tb_privilege_group] ADD CONSTRAINT [DF_tb_privilege_group_is_active] DEFAULT (1) FOR [is_active];
ALTER TABLE [dbo].[tb_privilege_group] ADD CONSTRAINT [DF_tb_privilege_group_is_delete] DEFAULT (0) FOR [is_delete];
ALTER TABLE [dbo].[tb_privilege_group] ADD CONSTRAINT [DF_tb_privilege_group_id_status] DEFAULT (N'ACTIVE') FOR [id_status];
GO

ALTER TABLE [dbo].[tb_privilege] ADD CONSTRAINT [DF_tb_privilege_prefix] DEFAULT (N'PRV') FOR [prefix];
ALTER TABLE [dbo].[tb_privilege] ADD CONSTRAINT [DF_tb_privilege_update_by] DEFAULT (N'System') FOR [update_by];
ALTER TABLE [dbo].[tb_privilege] ADD CONSTRAINT [DF_tb_privilege_update_date] DEFAULT (CONVERT([datetime],(sysdatetimeoffset() AT TIME ZONE 'SE Asia Standard Time'))) FOR [update_date];
ALTER TABLE [dbo].[tb_privilege] ADD CONSTRAINT [DF_tb_privilege_is_active] DEFAULT (1) FOR [is_active];
ALTER TABLE [dbo].[tb_privilege] ADD CONSTRAINT [DF_tb_privilege_is_delete] DEFAULT (0) FOR [is_delete];
ALTER TABLE [dbo].[tb_privilege] ADD CONSTRAINT [DF_tb_privilege_id_status] DEFAULT (N'ACTIVE') FOR [id_status];
ALTER TABLE [dbo].[tb_privilege] ADD CONSTRAINT [DF_tb_privilege_can_view] DEFAULT (0) FOR [can_view];
ALTER TABLE [dbo].[tb_privilege] ADD CONSTRAINT [DF_tb_privilege_can_insert] DEFAULT (0) FOR [can_insert];
ALTER TABLE [dbo].[tb_privilege] ADD CONSTRAINT [DF_tb_privilege_can_update] DEFAULT (0) FOR [can_update];
ALTER TABLE [dbo].[tb_privilege] ADD CONSTRAINT [DF_tb_privilege_can_delete] DEFAULT (0) FOR [can_delete];
GO
...
ALTER TABLE [dbo].[tb_discount_group] ADD CONSTRAINT [DF_tb_discount_group_prefix] DEFAULT (N'DCG') FOR [prefix];
ALTER TABLE [dbo].[tb_discount_group] ADD CONSTRAINT [DF_tb_discount_group_update_by] DEFAULT (N'System') FOR [update_by];
ALTER TABLE [dbo].[tb_discount_group] ADD CONSTRAINT [DF_tb_discount_group_update_date] DEFAULT (CONVERT([datetime],(sysdatetimeoffset() AT TIME ZONE 'SE Asia Standard Time'))) FOR [update_date];
ALTER TABLE [dbo].[tb_discount_group] ADD CONSTRAINT [DF_tb_discount_group_is_active] DEFAULT (1) FOR [is_active];
ALTER TABLE [dbo].[tb_discount_group] ADD CONSTRAINT [DF_tb_discount_group_is_delete] DEFAULT (0) FOR [is_delete];
ALTER TABLE [dbo].[tb_discount_group] ADD CONSTRAINT [DF_tb_discount_group_id_status] DEFAULT (N'ACTIVE') FOR [id_status];
GO

ALTER TABLE [dbo].[tb_price_rule] ADD CONSTRAINT [DF_tb_price_rule_prefix] DEFAULT (N'PRL') FOR [prefix];
ALTER TABLE [dbo].[tb_price_rule] ADD CONSTRAINT [DF_tb_price_rule_update_by] DEFAULT (N'System') FOR [update_by];
ALTER TABLE [dbo].[tb_price_rule] ADD CONSTRAINT [DF_tb_price_rule_update_date] DEFAULT (CONVERT([datetime],(sysdatetimeoffset() AT TIME ZONE 'SE Asia Standard Time'))) FOR [update_date];
ALTER TABLE [dbo].[tb_price_rule] ADD CONSTRAINT [DF_tb_price_rule_is_active] DEFAULT (1) FOR [is_active];
ALTER TABLE [dbo].[tb_price_rule] ADD CONSTRAINT [DF_tb_price_rule_is_delete] DEFAULT (0) FOR [is_delete];
ALTER TABLE [dbo].[tb_price_rule] ADD CONSTRAINT [DF_tb_price_rule_id_status] DEFAULT (N'ACTIVE') FOR [id_status];
ALTER TABLE [dbo].[tb_price_rule] ADD CONSTRAINT [DF_tb_price_rule_is_on_top] DEFAULT (0) FOR [is_on_top];
ALTER TABLE [dbo].[tb_price_rule] ADD CONSTRAINT [DF_tb_price_rule_priority] DEFAULT (0) FOR [priority];
GO
...
ALTER TABLE [dbo].[tb_price_rule] WITH CHECK ADD CONSTRAINT [CK_tb_price_rule_scope]
    CHECK ([rule_scope] IN (N'CUSTOMER_SKU', N'CUSTOMER', N'ROUTE', N'GROUP_SKU', N'GROUP', N'SKU'));

/* ปลายทางต้องตรงกับ scope — กันแถวกำพร้าที่ resolve แล้วไม่เจอใคร */
ALTER TABLE [dbo].[tb_price_rule] WITH CHECK ADD CONSTRAINT [CK_tb_price_rule_target] CHECK (
       ([rule_scope] = N'CUSTOMER_SKU' AND [ref_customer_auto] IS NOT NULL AND [ref_sku_auto] IS NOT NULL
                                       AND [ref_discount_group_auto] IS NULL AND [ref_route_auto] IS NULL)
    OR ([rule_scope] = N'CUSTOMER'     AND [ref_customer_auto] IS NOT NULL AND [ref_sku_auto] IS NULL
                                       AND [ref_discount_group_auto] IS NULL AND [ref_route_auto] IS NULL)
    OR ([rule_scope] = N'ROUTE'        AND [ref_route_auto] IS NOT NULL AND [ref_customer_auto] IS NULL
                                       AND [ref_discount_group_auto] IS NULL AND [ref_sku_auto] IS NULL)
    OR ([rule_scope] = N'GROUP_SKU'    AND [ref_discount_group_auto] IS NOT NULL AND [ref_sku_auto] IS NOT NULL
                                       AND [ref_customer_auto] IS NULL AND [ref_route_auto] IS NULL)
    OR ([rule_scope] = N'GROUP'        AND [ref_discount_group_auto] IS NOT NULL AND [ref_sku_auto] IS NULL
                                       AND [ref_customer_auto] IS NULL AND [ref_route_auto] IS NULL)
    OR ([rule_scope] = N'SKU'          AND [ref_sku_auto] IS NOT NULL AND [ref_customer_auto] IS NULL
                                       AND [ref_discount_group_auto] IS NULL AND [ref_route_auto] IS NULL)
);

/* มูลค่าต้องมีค่าเดียว : เป็น % หรือเป็นราคาสุทธิ ไม่ใช่ทั้งคู่ ไม่ใช่ไม่มีเลย */
ALTER TABLE [dbo].[tb_price_rule] WITH CHECK ADD CONSTRAINT [CK_tb_price_rule_value] CHECK (
    ([discount_percent] IS NULL AND [net_price] IS NOT NULL)
 OR ([discount_percent] IS NOT NULL AND [net_price] IS NULL)
);
ALTER TABLE [dbo].[tb_price_rule] WITH CHECK ADD CONSTRAINT [CK_tb_price_rule_percent]
    CHECK ([discount_percent] IS NULL OR ([discount_percent] >= 0 AND [discount_percent] <= 100));
ALTER TABLE [dbo].[tb_price_rule] WITH CHECK ADD CONSTRAINT [CK_tb_price_rule_net_price]
    CHECK ([net_price] IS NULL OR [net_price] >= 0);

/* on-top บวกทับผู้ชนะได้เฉพาะเมื่อเป็น % — ราคาสุทธิบวกทับไม่ได้ */
ALTER TABLE [dbo].[tb_price_rule] WITH CHECK ADD CONSTRAINT [CK_tb_price_rule_on_top]
    CHECK ([is_on_top] = 0 OR [net_price] IS NULL);

ALTER TABLE [dbo].[tb_price_rule] WITH CHECK ADD CONSTRAINT [CK_tb_price_rule_qty]
    CHECK ([min_qty] IS NULL OR [max_qty] IS NULL OR [max_qty] >= [min_qty]);
ALTER TABLE [dbo].[tb_price_rule] WITH CHECK ADD CONSTRAINT [CK_tb_price_rule_dates]
...
ALTER TABLE [dbo].[tb_customer] WITH CHECK ADD CONSTRAINT [FK_tb_customer_ref_discount_group_auto]
    FOREIGN KEY([ref_discount_group_auto]) REFERENCES [dbo].[tb_discount_group] ([autoID]);

ALTER TABLE [dbo].[tb_price_rule] WITH CHECK ADD CONSTRAINT [FK_tb_price_rule_ref_discount_group_auto]
    FOREIGN KEY([ref_discount_group_auto]) REFERENCES [dbo].[tb_discount_group] ([autoID]);
ALTER TABLE [dbo].[tb_price_rule] WITH CHECK ADD CONSTRAINT [FK_tb_price_rule_ref_customer_auto]
    FOREIGN KEY([ref_customer_auto]) REFERENCES [dbo].[tb_customer] ([autoID]);
ALTER TABLE [dbo].[tb_price_rule] WITH CHECK ADD CONSTRAINT [FK_tb_price_rule_ref_route_auto]
    FOREIGN KEY([ref_route_auto]) REFERENCES [dbo].[tb_route] ([autoID]);
ALTER TABLE [dbo].[tb_price_rule] WITH CHECK ADD CONSTRAINT [FK_tb_price_rule_ref_sku_auto]
    FOREIGN KEY([ref_sku_auto]) REFERENCES [dbo].[tb_product_sku] ([autoID]);
GO

ALTER TABLE [dbo].[tb_user_role] WITH CHECK ADD CONSTRAINT [FK_tb_user_role_ref_user_auto]
    FOREIGN KEY([ref_user_auto]) REFERENCES [dbo].[tb_users] ([autoID]);
ALTER TABLE [dbo].[tb_user_role] WITH CHECK ADD CONSTRAINT [FK_tb_user_role_ref_role_auto]
    FOREIGN KEY([ref_role_auto]) REFERENCES [dbo].[tb_role] ([autoID]);
ALTER TABLE [dbo].[tb_privilege] WITH CHECK ADD CONSTRAINT [FK_tb_privilege_ref_role_auto]
    FOREIGN KEY([ref_role_auto]) REFERENCES [dbo].[tb_role] ([autoID]);
ALTER TABLE [dbo].[tb_privilege] WITH CHECK ADD CONSTRAINT [FK_tb_privilege_ref_privilege_group_auto]
    FOREIGN KEY([ref_privilege_group_auto]) REFERENCES [dbo].[tb_privilege_group] ([autoID]);
GO
...
/****** TRIG_GENERATE_TB_DISCOUNT_GROUP_ID ******/
GO
CREATE TRIGGER [dbo].[TRIG_GENERATE_TB_DISCOUNT_GROUP_ID] ON [dbo].[tb_discount_group] AFTER INSERT AS
BEGIN
    SET NOCOUNT ON;
    DECLARE @cnt INT = (SELECT COUNT(*) FROM inserted WHERE [discount_group_id] IS NULL);
    IF @cnt = 0 RETURN;
    DECLARE @startNo INT, @series NVARCHAR(1);
    EXEC [dbo].[USP_ALLOCATE_BUSINESS_ID_BLOCK] @TableName = N'tb_discount_group', @BlockSize = @cnt,
         @StartNo = @startNo OUTPUT, @Series = @series OUTPUT;
    ;WITH src AS (SELECT autoID, prefix, ROW_NUMBER() OVER (ORDER BY autoID) - 1 AS rn
                    FROM inserted WHERE [discount_group_id] IS NULL)
    UPDATE t SET t.[discount_group_id] = src.prefix + @series
                + RIGHT(N'000000' + CAST(@startNo + src.rn AS NVARCHAR(10)), 6)
      FROM dbo.tb_discount_group t INNER JOIN src ON t.autoID = src.autoID;
END
GO

/****** TRIG_AUTO_UPDATE_DATE_TB_DISCOUNT_GROUP ******/
GO
CREATE TRIGGER [dbo].[TRIG_AUTO_UPDATE_DATE_TB_DISCOUNT_GROUP] ON [dbo].[tb_discount_group] AFTER UPDATE AS
BEGIN
    SET NOCOUNT ON;
    IF TRIGGER_NESTLEVEL() > 1 RETURN;
    UPDATE t SET t.update_date = CAST(SYSDATETIMEOFFSET() AT TIME ZONE 'SE Asia Standard Time' AS DATETIME)
      FROM dbo.tb_discount_group t INNER JOIN inserted i ON t.autoID = i.autoID;
END
GO

/****** TRIG_SYNC_STATUS_TB_DISCOUNT_GROUP ******/
GO
CREATE TRIGGER [dbo].[TRIG_SYNC_STATUS_TB_DISCOUNT_GROUP] ON [dbo].[tb_discount_group] AFTER UPDATE AS
BEGIN
    SET NOCOUNT ON;
    IF TRIGGER_NESTLEVEL() > 1 RETURN;
    IF NOT UPDATE([is_delete]) RETURN;
...
/****** TRIG_BLOCK_DELETE_TB_DISCOUNT_GROUP ******/
GO
CREATE TRIGGER [dbo].[TRIG_BLOCK_DELETE_TB_DISCOUNT_GROUP] ON [dbo].[tb_discount_group] INSTEAD OF DELETE AS
BEGIN
    SET NOCOUNT ON;
    UPDATE t SET t.is_delete = 1, t.is_active = 0, t.id_status = N'DELETED',
                 t.update_date = CAST(SYSDATETIMEOFFSET() AT TIME ZONE 'SE Asia Standard Time' AS DATETIME)
      FROM dbo.tb_discount_group t INNER JOIN deleted d ON t.autoID = d.autoID;
END
GO

/****** TRIG_GENERATE_TB_PRICE_RULE_ID ******/
GO
CREATE TRIGGER [dbo].[TRIG_GENERATE_TB_PRICE_RULE_ID] ON [dbo].[tb_price_rule] AFTER INSERT AS
BEGIN
    SET NOCOUNT ON;
    DECLARE @cnt INT = (SELECT COUNT(*) FROM inserted WHERE [price_rule_id] IS NULL);
    IF @cnt = 0 RETURN;
    DECLARE @startNo INT, @series NVARCHAR(1);
    EXEC [dbo].[USP_ALLOCATE_BUSINESS_ID_BLOCK] @TableName = N'tb_price_rule', @BlockSize = @cnt,
         @StartNo = @startNo OUTPUT, @Series = @series OUTPUT;
    ;WITH src AS (SELECT autoID, prefix, ROW_NUMBER() OVER (ORDER BY autoID) - 1 AS rn
                    FROM inserted WHERE [price_rule_id] IS NULL)
    UPDATE t SET t.[price_rule_id] = src.prefix + @series
                + RIGHT(N'000000' + CAST(@startNo + src.rn AS NVARCHAR(10)), 6)
      FROM dbo.tb_price_rule t INNER JOIN src ON t.autoID = src.autoID;
END
GO

/****** TRIG_AUTO_UPDATE_DATE_TB_PRICE_RULE ******/
GO
CREATE TRIGGER [dbo].[TRIG_AUTO_UPDATE_DATE_TB_PRICE_RULE] ON [dbo].[tb_price_rule] AFTER UPDATE AS
BEGIN
    SET NOCOUNT ON;
    IF TRIGGER_NESTLEVEL() > 1 RETURN;
    UPDATE t SET t.update_date = CAST(SYSDATETIMEOFFSET() AT TIME ZONE 'SE Asia Standard Time' AS DATETIME)
      FROM dbo.tb_price_rule t INNER JOIN inserted i ON t.autoID = i.autoID;
END
GO

/****** TRIG_SYNC_STATUS_TB_PRICE_RULE ******/
GO
CREATE TRIGGER [dbo].[TRIG_SYNC_STATUS_TB_PRICE_RULE] ON [dbo].[tb_price_rule] AFTER UPDATE AS
BEGIN
    SET NOCOUNT ON;
    IF TRIGGER_NESTLEVEL() > 1 RETURN;
    IF NOT UPDATE([is_delete]) RETURN;
...
/****** TRIG_BLOCK_DELETE_TB_PRICE_RULE ******/
GO
CREATE TRIGGER [dbo].[TRIG_BLOCK_DELETE_TB_PRICE_RULE] ON [dbo].[tb_price_rule] INSTEAD OF DELETE AS
BEGIN
    SET NOCOUNT ON;
    UPDATE t SET t.is_delete = 1, t.is_active = 0, t.id_status = N'DELETED',
                 t.update_date = CAST(SYSDATETIMEOFFSET() AT TIME ZONE 'SE Asia Standard Time' AS DATETIME)
      FROM dbo.tb_price_rule t INNER JOIN deleted d ON t.autoID = d.autoID;
END
GO

/****** TRIG_BLOCK_OVERLAP_TB_PRICE_RULE  --  กันช่วงวันซ้อนกันบนปลายทางเดียวกัน ******/
GO
CREATE TRIGGER [dbo].[TRIG_BLOCK_OVERLAP_TB_PRICE_RULE] ON [dbo].[tb_price_rule] AFTER INSERT, UPDATE AS
BEGIN
    SET NOCOUNT ON;
    IF TRIGGER_NESTLEVEL() > 1 RETURN;

    /* CHECK constraint มองได้แค่แถวเดียว จึงกันช่วงวันทับซ้อนไม่ได้ ต้องเป็น Trigger
       ทับซ้อน = scope + ปลายทาง + ขั้นบันได + is_on_top เดียวกัน และช่วงวันคาบเกี่ยว
       (NULL = ไม่มีขอบ จึงคาบเกี่ยวเสมอ) */
    IF EXISTS (
        SELECT 1
          FROM inserted i
          JOIN dbo.tb_price_rule r
            ON  r.autoID <> i.autoID
            AND r.is_delete = 0 AND r.is_active = 1
            AND r.rule_scope = i.rule_scope
            AND r.is_on_top  = i.is_on_top
            AND ISNULL(r.ref_discount_group_auto, 0) = ISNULL(i.ref_discount_group_auto, 0)
            AND ISNULL(r.ref_customer_auto, 0)       = ISNULL(i.ref_customer_auto, 0)
            AND ISNULL(r.ref_route_auto, 0)          = ISNULL(i.ref_route_auto, 0)
            AND ISNULL(r.ref_sku_auto, 0)            = ISNULL(i.ref_sku_auto, 0)
            AND ISNULL(r.min_qty, 0)                 = ISNULL(i.min_qty, 0)
            AND ISNULL(r.max_qty, 2147483647)        = ISNULL(i.max_qty, 2147483647)
            AND ISNULL(r.start_date, '1900-01-01')  <= ISNULL(i.end_date,   '9999-12-31')
            AND ISNULL(r.end_date,   '9999-12-31')  >= ISNULL(i.start_date, '1900-01-01')
         WHERE i.is_delete = 0 AND i.is_active = 1
    )
        /* THROW ไม่ใช่ RAISERROR + ROLLBACK : ROLLBACK ในทริกเกอร์ทำให้ client ได้ error 3609
           (ธุรกรรมจบในทริกเกอร์) แทนเลข 50000 ของเราเอง แล้ว PenbunAPI แปลเป็น INTERNAL
           THROW ม้วนธุรกรรมกลับให้เหมือนกัน แต่ยังส่งเลข 50000 พร้อมข้อความไทยออกไป
           ชั้นบนจึงตอบ BUSINESS_RULE ได้ตามสัญญาใน httpx/sqlerr.go */
        THROW 50000, N'PRICE_RULE_OVERLAP: มีกฎส่วนลดที่ปลายทางและช่วงวันซ้อนกันอยู่แล้ว', 1;
END
GO

/* ═══════════ RBAC (v12) ═══════════ */
/****** TRIG_GENERATE_TB_ROLE_ID ******/
GO
CREATE TRIGGER [dbo].[TRIG_GENERATE_TB_ROLE_ID] ON [dbo].[tb_role] AFTER INSERT AS
BEGIN
    SET NOCOUNT ON;
    DECLARE @cnt INT = (SELECT COUNT(*) FROM inserted WHERE [role_id] IS NULL);
    IF @cnt = 0 RETURN;
    DECLARE @startNo INT, @series NVARCHAR(1);
    EXEC [dbo].[USP_ALLOCATE_BUSINESS_ID_BLOCK] @TableName = N'tb_role', @BlockSize = @cnt,
         @StartNo = @startNo OUTPUT, @Series = @series OUTPUT;
    ;WITH src AS (SELECT autoID, prefix, ROW_NUMBER() OVER (ORDER BY autoID) - 1 AS rn
                    FROM inserted WHERE [role_id] IS NULL)
    UPDATE t SET t.[role_id] = src.prefix + @series
                + RIGHT(N'000000' + CAST(@startNo + src.rn AS NVARCHAR(10)), 6)
      FROM dbo.tb_role t INNER JOIN src ON t.autoID = src.autoID;
END
GO
/****** TRIG_AUTO_UPDATE_DATE_TB_ROLE ******/
GO
CREATE TRIGGER [dbo].[TRIG_AUTO_UPDATE_DATE_TB_ROLE] ON [dbo].[tb_role] AFTER UPDATE AS
BEGIN
    SET NOCOUNT ON;
    IF TRIGGER_NESTLEVEL() > 1 RETURN;
    UPDATE t SET t.update_date = CAST(SYSDATETIMEOFFSET() AT TIME ZONE 'SE Asia Standard Time' AS DATETIME)
      FROM dbo.tb_role t INNER JOIN inserted i ON t.autoID = i.autoID;
END
GO
/****** TRIG_SYNC_STATUS_TB_ROLE ******/
GO
CREATE TRIGGER [dbo].[TRIG_SYNC_STATUS_TB_ROLE] ON [dbo].[tb_role] AFTER UPDATE AS
BEGIN
    SET NOCOUNT ON;
    IF TRIGGER_NESTLEVEL() > 1 RETURN;
    IF NOT UPDATE([is_delete]) RETURN;
...
END
GO
/****** TRIG_BLOCK_DELETE_TB_ROLE ******/
GO
CREATE TRIGGER [dbo].[TRIG_BLOCK_DELETE_TB_ROLE] ON [dbo].[tb_role] INSTEAD OF DELETE AS
BEGIN
    SET NOCOUNT ON;
    UPDATE t SET t.is_delete = 1, t.is_active = 0, t.id_status = N'DELETED',
                 t.update_date = CAST(SYSDATETIMEOFFSET() AT TIME ZONE 'SE Asia Standard Time' AS DATETIME)
      FROM dbo.tb_role t INNER JOIN deleted d ON t.autoID = d.autoID;
END
GO
/****** TRIG_GENERATE_TB_USER_ROLE_ID ******/
GO
CREATE TRIGGER [dbo].[TRIG_GENERATE_TB_USER_ROLE_ID] ON [dbo].[tb_user_role] AFTER INSERT AS
BEGIN
    SET NOCOUNT ON;
    DECLARE @cnt INT = (SELECT COUNT(*) FROM inserted WHERE [user_role_id] IS NULL);
    IF @cnt = 0 RETURN;
    DECLARE @startNo INT, @series NVARCHAR(1);
    EXEC [dbo].[USP_ALLOCATE_BUSINESS_ID_BLOCK] @TableName = N'tb_user_role', @BlockSize = @cnt,
         @StartNo = @startNo OUTPUT, @Series = @series OUTPUT;
    ;WITH src AS (SELECT autoID, prefix, ROW_NUMBER() OVER (ORDER BY autoID) - 1 AS rn
                    FROM inserted WHERE [user_role_id] IS NULL)
    UPDATE t SET t.[user_role_id] = src.prefix + @series
                + RIGHT(N'000000' + CAST(@startNo + src.rn AS NVARCHAR(10)), 6)
      FROM dbo.tb_user_role t INNER JOIN src ON t.autoID = src.autoID;
END
GO
/****** TRIG_AUTO_UPDATE_DATE_TB_USER_ROLE ******/
GO
CREATE TRIGGER [dbo].[TRIG_AUTO_UPDATE_DATE_TB_USER_ROLE] ON [dbo].[tb_user_role] AFTER UPDATE AS
BEGIN
    SET NOCOUNT ON;
    IF TRIGGER_NESTLEVEL() > 1 RETURN;
    UPDATE t SET t.update_date = CAST(SYSDATETIMEOFFSET() AT TIME ZONE 'SE Asia Standard Time' AS DATETIME)
      FROM dbo.tb_user_role t INNER JOIN inserted i ON t.autoID = i.autoID;
END
GO
/****** TRIG_SYNC_STATUS_TB_USER_ROLE ******/
GO
CREATE TRIGGER [dbo].[TRIG_SYNC_STATUS_TB_USER_ROLE] ON [dbo].[tb_user_role] AFTER UPDATE AS
BEGIN
    SET NOCOUNT ON;
    IF TRIGGER_NESTLEVEL() > 1 RETURN;
    IF NOT UPDATE([is_delete]) RETURN;
...
END
GO
/****** TRIG_BLOCK_DELETE_TB_USER_ROLE ******/
GO
CREATE TRIGGER [dbo].[TRIG_BLOCK_DELETE_TB_USER_ROLE] ON [dbo].[tb_user_role] INSTEAD OF DELETE AS
BEGIN
    SET NOCOUNT ON;
    UPDATE t SET t.is_delete = 1, t.is_active = 0, t.id_status = N'DELETED',
                 t.update_date = CAST(SYSDATETIMEOFFSET() AT TIME ZONE 'SE Asia Standard Time' AS DATETIME)
      FROM dbo.tb_user_role t INNER JOIN deleted d ON t.autoID = d.autoID;
END
GO
/****** TRIG_GENERATE_TB_PRIVILEGE_GROUP_ID ******/
GO
CREATE TRIGGER [dbo].[TRIG_GENERATE_TB_PRIVILEGE_GROUP_ID] ON [dbo].[tb_privilege_group] AFTER INSERT AS
BEGIN
    SET NOCOUNT ON;
    DECLARE @cnt INT = (SELECT COUNT(*) FROM inserted WHERE [privilege_group_id] IS NULL);
    IF @cnt = 0 RETURN;
    DECLARE @startNo INT, @series NVARCHAR(1);
    EXEC [dbo].[USP_ALLOCATE_BUSINESS_ID_BLOCK] @TableName = N'tb_privilege_group', @BlockSize = @cnt,
         @StartNo = @startNo OUTPUT, @Series = @series OUTPUT;
    ;WITH src AS (SELECT autoID, prefix, ROW_NUMBER() OVER (ORDER BY autoID) - 1 AS rn
                    FROM inserted WHERE [privilege_group_id] IS NULL)
    UPDATE t SET t.[privilege_group_id] = src.prefix + @series
                + RIGHT(N'000000' + CAST(@startNo + src.rn AS NVARCHAR(10)), 6)
      FROM dbo.tb_privilege_group t INNER JOIN src ON t.autoID = src.autoID;
END
GO
/****** TRIG_AUTO_UPDATE_DATE_TB_PRIVILEGE_GROUP ******/
GO
CREATE TRIGGER [dbo].[TRIG_AUTO_UPDATE_DATE_TB_PRIVILEGE_GROUP] ON [dbo].[tb_privilege_group] AFTER UPDATE AS
BEGIN
    SET NOCOUNT ON;
    IF TRIGGER_NESTLEVEL() > 1 RETURN;
    UPDATE t SET t.update_date = CAST(SYSDATETIMEOFFSET() AT TIME ZONE 'SE Asia Standard Time' AS DATETIME)
      FROM dbo.tb_privilege_group t INNER JOIN inserted i ON t.autoID = i.autoID;
END
GO
/****** TRIG_SYNC_STATUS_TB_PRIVILEGE_GROUP ******/
GO
CREATE TRIGGER [dbo].[TRIG_SYNC_STATUS_TB_PRIVILEGE_GROUP] ON [dbo].[tb_privilege_group] AFTER UPDATE AS
BEGIN
    SET NOCOUNT ON;
    IF TRIGGER_NESTLEVEL() > 1 RETURN;
    IF NOT UPDATE([is_delete]) RETURN;
...
END
GO
/****** TRIG_BLOCK_DELETE_TB_PRIVILEGE_GROUP ******/
GO
CREATE TRIGGER [dbo].[TRIG_BLOCK_DELETE_TB_PRIVILEGE_GROUP] ON [dbo].[tb_privilege_group] INSTEAD OF DELETE AS
BEGIN
    SET NOCOUNT ON;
    UPDATE t SET t.is_delete = 1, t.is_active = 0, t.id_status = N'DELETED',
                 t.update_date = CAST(SYSDATETIMEOFFSET() AT TIME ZONE 'SE Asia Standard Time' AS DATETIME)
      FROM dbo.tb_privilege_group t INNER JOIN deleted d ON t.autoID = d.autoID;
END
GO
/****** TRIG_GENERATE_TB_PRIVILEGE_ID ******/
GO
CREATE TRIGGER [dbo].[TRIG_GENERATE_TB_PRIVILEGE_ID] ON [dbo].[tb_privilege] AFTER INSERT AS
BEGIN
    SET NOCOUNT ON;
    DECLARE @cnt INT = (SELECT COUNT(*) FROM inserted WHERE [privilege_id] IS NULL);
    IF @cnt = 0 RETURN;
    DECLARE @startNo INT, @series NVARCHAR(1);
    EXEC [dbo].[USP_ALLOCATE_BUSINESS_ID_BLOCK] @TableName = N'tb_privilege', @BlockSize = @cnt,
         @StartNo = @startNo OUTPUT, @Series = @series OUTPUT;
    ;WITH src AS (SELECT autoID, prefix, ROW_NUMBER() OVER (ORDER BY autoID) - 1 AS rn
                    FROM inserted WHERE [privilege_id] IS NULL)
    UPDATE t SET t.[privilege_id] = src.prefix + @series
                + RIGHT(N'000000' + CAST(@startNo + src.rn AS NVARCHAR(10)), 6)
      FROM dbo.tb_privilege t INNER JOIN src ON t.autoID = src.autoID;
END
GO
/****** TRIG_AUTO_UPDATE_DATE_TB_PRIVILEGE ******/
GO
CREATE TRIGGER [dbo].[TRIG_AUTO_UPDATE_DATE_TB_PRIVILEGE] ON [dbo].[tb_privilege] AFTER UPDATE AS
BEGIN
    SET NOCOUNT ON;
    IF TRIGGER_NESTLEVEL() > 1 RETURN;
    UPDATE t SET t.update_date = CAST(SYSDATETIMEOFFSET() AT TIME ZONE 'SE Asia Standard Time' AS DATETIME)
      FROM dbo.tb_privilege t INNER JOIN inserted i ON t.autoID = i.autoID;
END
GO
/****** TRIG_SYNC_STATUS_TB_PRIVILEGE ******/
GO
CREATE TRIGGER [dbo].[TRIG_SYNC_STATUS_TB_PRIVILEGE] ON [dbo].[tb_privilege] AFTER UPDATE AS
BEGIN
    SET NOCOUNT ON;
    IF TRIGGER_NESTLEVEL() > 1 RETURN;
    IF NOT UPDATE([is_delete]) RETURN;
...
END
GO
/****** TRIG_BLOCK_DELETE_TB_PRIVILEGE ******/
GO
CREATE TRIGGER [dbo].[TRIG_BLOCK_DELETE_TB_PRIVILEGE] ON [dbo].[tb_privilege] INSTEAD OF DELETE AS
BEGIN
    SET NOCOUNT ON;
    UPDATE t SET t.is_delete = 1, t.is_active = 0, t.id_status = N'DELETED',
                 t.update_date = CAST(SYSDATETIMEOFFSET() AT TIME ZONE 'SE Asia Standard Time' AS DATETIME)
      FROM dbo.tb_privilege t INNER JOIN deleted d ON t.autoID = d.autoID;
END
GO
...
CREATE UNIQUE NONCLUSTERED INDEX [UQ_tb_discount_group_id] ON [dbo].[tb_discount_group]([discount_group_id]) WHERE [discount_group_id] IS NOT NULL;
CREATE UNIQUE NONCLUSTERED INDEX [UQ_tb_discount_group_code] ON [dbo].[tb_discount_group]([group_code]) WHERE [is_delete] = 0;
CREATE UNIQUE NONCLUSTERED INDEX [UQ_tb_price_rule_id] ON [dbo].[tb_price_rule]([price_rule_id]) WHERE [price_rule_id] IS NOT NULL;

CREATE NONCLUSTERED INDEX [IX_tb_price_rule_lookup] ON [dbo].[tb_price_rule]([rule_scope], [is_active], [start_date], [end_date])
    INCLUDE ([ref_discount_group_auto], [ref_customer_auto], [ref_route_auto], [ref_sku_auto],
             [discount_percent], [net_price], [min_qty], [max_qty], [is_on_top], [priority]) WHERE [is_delete] = 0;
CREATE NONCLUSTERED INDEX [IX_tb_price_rule_ref_customer_auto] ON [dbo].[tb_price_rule]([ref_customer_auto]) WHERE [ref_customer_auto] IS NOT NULL;
CREATE NONCLUSTERED INDEX [IX_tb_price_rule_ref_sku_auto] ON [dbo].[tb_price_rule]([ref_sku_auto]) WHERE [ref_sku_auto] IS NOT NULL;
CREATE NONCLUSTERED INDEX [IX_tb_customer_ref_discount_group_auto] ON [dbo].[tb_customer]([ref_discount_group_auto]) WHERE [ref_discount_group_auto] IS NOT NULL;
GO

-- 8.x RBAC (v12)
CREATE UNIQUE NONCLUSTERED INDEX [UQ_tb_role_id] ON [dbo].[tb_role]([role_id]) WHERE [role_id] IS NOT NULL;
CREATE UNIQUE NONCLUSTERED INDEX [UQ_tb_user_role_id] ON [dbo].[tb_user_role]([user_role_id]) WHERE [user_role_id] IS NOT NULL;
CREATE UNIQUE NONCLUSTERED INDEX [UQ_tb_privilege_group_id] ON [dbo].[tb_privilege_group]([privilege_group_id]) WHERE [privilege_group_id] IS NOT NULL;
CREATE UNIQUE NONCLUSTERED INDEX [UQ_tb_privilege_id] ON [dbo].[tb_privilege]([privilege_id]) WHERE [privilege_id] IS NOT NULL;

-- รหัสบทบาทและกลุ่มสิทธิ์ห้ามซ้ำในแถวที่ยังไม่ถูกลบ
CREATE UNIQUE NONCLUSTERED INDEX [UQ_tb_role_code] ON [dbo].[tb_role]([role_code]) WHERE [is_delete] = 0;
CREATE UNIQUE NONCLUSTERED INDEX [UQ_tb_privilege_group_code] ON [dbo].[tb_privilege_group]([group_code]) WHERE [is_delete] = 0;

-- หนึ่งบทบาทมีสิทธิ์ต่อ resource ได้แถวเดียว และผู้ใช้ถือบทบาทเดิมซ้ำไม่ได้
CREATE UNIQUE NONCLUSTERED INDEX [UQ_tb_privilege_role_resource] ON [dbo].[tb_privilege]([ref_role_auto],[resource_code]) WHERE [is_delete] = 0;
CREATE UNIQUE NONCLUSTERED INDEX [UQ_tb_user_role_pair] ON [dbo].[tb_user_role]([ref_user_auto],[ref_role_auto]) WHERE [is_delete] = 0;
GO
...
CREATE VIEW [dbo].[vw_discount_group] AS
SELECT  g.autoID AS discount_group_auto,
        g.discount_group_id, g.group_code, g.group_name, g.description,
        (SELECT COUNT(*) FROM dbo.tb_customer c
          WHERE c.ref_discount_group_auto = g.autoID AND c.is_delete = 0) AS customer_count,
        (SELECT COUNT(*) FROM dbo.tb_price_rule r
          WHERE r.ref_discount_group_auto = g.autoID AND r.is_delete = 0) AS rule_count,
        g.is_active, g.id_status, g.update_by, g.update_date
  FROM dbo.tb_discount_group g
 WHERE g.is_delete = 0;
GO

CREATE VIEW [dbo].[vw_price_rule] AS
SELECT  r.autoID AS price_rule_auto,
        r.price_rule_id, r.rule_code, r.rule_name, r.rule_scope,
        /* ปลายทางคืนเป็น Business ID : PenbunAPI แปลงกลับเป็น autoID ตอนเขียน
           และหน้าจอต้องใช้ค่านี้เติมช่อง ref บนฟอร์ม */
        g.discount_group_id, g.group_code AS discount_group_code, g.group_name AS discount_group_name,
        c.customer_id, c.customer_code, c.customer_name,
        rt.route_id, rt.route_code, rt.route_name,
        s.sku_id, s.sku_code, p.product_name,
        /* ชื่อปลายทางรวมเป็นคอลัมน์เดียว — ตารางบนหน้าจอจะได้ไม่ต้องมีสี่ช่องที่ว่างสามช่องเสมอ */
        COALESCE(c.customer_name, rt.route_name, g.group_name) AS target_name,
        r.discount_percent, r.net_price, r.min_qty, r.max_qty,
        r.is_on_top, r.priority, r.start_date, r.end_date, r.description,
        r.is_active, r.id_status, r.update_by, r.update_date
  FROM dbo.tb_price_rule r
  LEFT JOIN dbo.tb_discount_group g ON g.autoID = r.ref_discount_group_auto
  LEFT JOIN dbo.tb_customer       c ON c.autoID = r.ref_customer_auto
  LEFT JOIN dbo.tb_route          rt ON rt.autoID = r.ref_route_auto
  LEFT JOIN dbo.tb_product_sku    s ON s.autoID = r.ref_sku_auto
  LEFT JOIN dbo.tb_product        p ON p.autoID = s.ref_product_auto
 WHERE r.is_delete = 0;
GO

/* vw_customer : กลุ่มส่วนลดคืนมาเป็น Business ID + ชื่อ ไม่ใช่ข้อความอิสระแบบ v8 */
...
SELECT  c.autoID AS customer_auto,
        ...
        c.credit_limit, c.credit_term_day, c.is_vat, c.invoice_format,
        c.ref_discount_group_auto, g.discount_group_id, g.group_code AS discount_group_code,
        g.group_name AS discount_group_name,
  ...
  LEFT  JOIN dbo.tb_discount_group g ON g.autoID = c.ref_discount_group_auto
 WHERE c.is_delete = 0;
GO
...
/* ─────────────────────────────────────────────────────────────────────────────
   ...
   PenbunAPI เคยเขียน SELECT ชุดนี้ฝังไว้ในแพ็กเกจ internal.resources พร้อม
...
SELECT  autoID AS company_auto,
        ...
        tax_id, branch_code, address, sub_district, district, province, zip_code,
        phone, email, website,
...
SELECT  w.autoID AS warehouse_auto,
        ...
        w.address, w.sub_district, w.district, w.province, w.zip_code, w.description,
...
/* ─────────────────────────────────────────────────────────────────────────────
   v11 : View ของผู้ใช้งาน
   user_password และ counting_password_fail ไม่อยู่ใน SELECT list โดยตั้งใจ
   หน้าจอผู้ใช้อ่านผ่าน View นี้  ส่วน auth ยังอ่าน tb_users ตรงเหมือนเดิม
   เพราะต้องการ hash ที่ View นี้ไม่คืนให้
   ───────────────────────────────────────────────────────────────────────── */
...
/* ── RBAC (v12) ───────────────────────────────────────────────────────────── */
CREATE VIEW [dbo].[vw_role] AS
SELECT  r.autoID AS role_auto,
        r.role_id, r.role_code, r.role_name, r.description, r.is_system,
        r.is_active, r.id_status, r.update_by, r.update_date
  FROM dbo.tb_role r
 WHERE r.is_delete = 0;
GO

CREATE VIEW [dbo].[vw_privilege] AS
SELECT  pv.autoID AS privilege_auto,
        pv.privilege_id,
        r.autoID AS role_auto, r.role_id, r.role_code, r.role_name,
        g.autoID AS privilege_group_auto, g.privilege_group_id, g.group_code, g.group_name,
        pv.resource_code, pv.can_view, pv.can_insert, pv.can_update, pv.can_delete,
        pv.is_active, pv.id_status, pv.update_by, pv.update_date
  FROM dbo.tb_privilege pv
  INNER JOIN dbo.tb_role            r ON r.autoID = pv.ref_role_auto
  INNER JOIN dbo.tb_privilege_group g ON g.autoID = pv.ref_privilege_group_auto
 WHERE pv.is_delete = 0;
GO

/* vw_user_privilege — สิทธิ์ผลลัพธ์ของผู้ใช้หนึ่งคน อ่านครั้งเดียวจบ
   ผู้ใช้ถือได้หลายบทบาท (ER หน้า 33 = M..N) สิทธิ์จึงรวมแบบ union :
   บทบาทใดให้ผ่าน ก็ผ่าน  MAX ทำหน้าที่นั้นให้แล้ว
   แถวที่ถูกลบหรือปิดใช้งานไม่ถูกนับ ทั้งฝั่งผู้ใช้ บทบาท และสิทธิ์
   GET /auth/me อ่าน View นี้แล้วส่งกลับเป็นรายการสิทธิ์ได้ตรง ๆ */
CREATE VIEW [dbo].[vw_user_privilege] AS
SELECT  u.autoID AS user_auto, u.user_id, u.user_name,
        pv.resource_code,
        MAX(CAST(pv.can_view   AS INT)) AS can_view,
        MAX(CAST(pv.can_insert AS INT)) AS can_insert,
        MAX(CAST(pv.can_update AS INT)) AS can_update,
        MAX(CAST(pv.can_delete AS INT)) AS can_delete
  FROM dbo.tb_users u
  INNER JOIN dbo.tb_user_role ur ON ur.ref_user_auto = u.autoID AND ur.is_delete = 0 AND ur.is_active = 1
  INNER JOIN dbo.tb_role       r ON r.autoID = ur.ref_role_auto AND r.is_delete = 0 AND r.is_active = 1
  INNER JOIN dbo.tb_privilege pv ON pv.ref_role_auto = r.autoID AND pv.is_delete = 0 AND pv.is_active = 1
 WHERE u.is_delete = 0
 GROUP BY u.autoID, u.user_id, u.user_name, pv.resource_code;
GO
...
/* UFN_RESOLVE_DISCOUNT — คืนหนึ่งแถวเสมอ ต่อ (ลูกค้า × SKU × จำนวน × วันที่)

   base   : กฎที่ is_on_top = 0 แข่งกัน เจาะจงกว่าชนะ
            CUSTOMER_SKU(60) > CUSTOMER(50) > ROUTE(40) > GROUP_SKU(30) > GROUP(20) > SKU(10)
            เสมอกัน -> priority มากชนะ -> min_qty สูงชนะ -> แถวใหม่ชนะ
   on_top : กฎที่ is_on_top = 1 ทุกแถวที่เข้าเกณฑ์ บวกกันแล้วบวกทับ base
   ชั้นสุดท้าย : ไม่เจอกฎเลย -> tb_book.customer_discount_percent -> ไม่มีอีก -> 0

   source_scope บอกว่าเลขมาจากชั้นไหน  หน้าจอใบสั่งขายต้องแสดงค่านี้ควบไปกับตัวเลข
   ไม่ใช่ช่องเปล่าที่ไม่มีที่มา (PENBUN-TODO §8.2) */
CREATE FUNCTION [dbo].[UFN_RESOLVE_DISCOUNT]
(
    @customer_auto INT,
    @sku_auto      INT,
    @qty           INT,
...
(
    WITH ctx AS (
        SELECT  c.autoID AS customer_auto,
                c.ref_discount_group_auto AS group_auto,
                (SELECT TOP 1 cr.ref_route_auto
                   FROM dbo.tb_customer_route cr
                  WHERE cr.ref_customer_auto = c.autoID AND cr.is_delete = 0 AND cr.is_active = 1
                  ORDER BY cr.is_primary DESC, cr.autoID) AS route_auto
          FROM dbo.tb_customer c
         WHERE c.autoID = @customer_auto
    ),
    hit AS (
        SELECT  r.autoID, r.rule_scope, r.discount_percent, r.net_price,
                r.is_on_top, r.priority, r.min_qty,
                CASE r.rule_scope
                     WHEN N'CUSTOMER_SKU' THEN 60
                     WHEN N'CUSTOMER'     THEN 50
                     WHEN N'ROUTE'        THEN 40
                     WHEN N'GROUP_SKU'    THEN 30
                     WHEN N'GROUP'        THEN 20
                     ELSE 10
                END AS specificity
          FROM dbo.tb_price_rule r
         CROSS JOIN ctx
         WHERE r.is_delete = 0 AND r.is_active = 1
           AND (r.start_date IS NULL OR r.start_date <= @doc_date)
           AND (r.end_date   IS NULL OR r.end_date   >= @doc_date)
           AND (r.min_qty    IS NULL OR @qty >= r.min_qty)
           AND (r.max_qty    IS NULL OR @qty <= r.max_qty)
           AND (   (r.rule_scope = N'CUSTOMER_SKU' AND r.ref_customer_auto = ctx.customer_auto AND r.ref_sku_auto = @sku_auto)
                OR (r.rule_scope = N'CUSTOMER'     AND r.ref_customer_auto = ctx.customer_auto)
                OR (r.rule_scope = N'ROUTE'        AND r.ref_route_auto    = ctx.route_auto)
                OR (r.rule_scope = N'GROUP_SKU'    AND r.ref_discount_group_auto = ctx.group_auto AND r.ref_sku_auto = @sku_auto)
                OR (r.rule_scope = N'GROUP'        AND r.ref_discount_group_auto = ctx.group_auto)
                OR (r.rule_scope = N'SKU'          AND r.ref_sku_auto      = @sku_auto)
               )
    ),
    base AS (
        SELECT TOP 1 autoID, rule_scope, discount_percent, net_price
          FROM hit
         WHERE is_on_top = 0
         ORDER BY specificity DESC, priority DESC, ISNULL(min_qty, 0) DESC, autoID DESC
    ),
    on_top AS (
        SELECT ISNULL(SUM(discount_percent), 0) AS pct FROM hit WHERE is_on_top = 1
    ),
    fallback AS (
        SELECT TOP 1 bk.customer_discount_percent AS pct
          FROM dbo.tb_product_sku s
          JOIN dbo.tb_book bk ON bk.ref_product_auto = s.ref_product_auto AND bk.is_delete = 0
         WHERE s.autoID = @sku_auto
         ORDER BY bk.autoID
    )
    SELECT
        /* ส่วนลดรวมเป็น % : ใช้ได้เมื่อ base ไม่ได้กำหนดราคาสุทธิมา */
        CASE WHEN (SELECT net_price FROM base) IS NOT NULL THEN NULL
             ELSE CASE WHEN COALESCE((SELECT discount_percent FROM base),
                                     (SELECT pct FROM fallback), 0)
                       + (SELECT pct FROM on_top) > 100
                       THEN 100
                       ELSE COALESCE((SELECT discount_percent FROM base),
                                     (SELECT pct FROM fallback), 0)
                          + (SELECT pct FROM on_top)
                  END
        END AS discount_percent,
        /* ราคาสุทธิต่อหน่วย : มีค่าเฉพาะเมื่อกฎที่ชนะกำหนดราคามาตรง ๆ (legacy 1.8) */
        (SELECT net_price FROM base) AS net_price,
        COALESCE((SELECT discount_percent FROM base), 0) AS base_percent,
        (SELECT pct FROM on_top)                          AS on_top_percent,
        COALESCE((SELECT rule_scope FROM base),
                 CASE WHEN (SELECT pct FROM fallback) IS NOT NULL THEN N'BOOK_DEFAULT' ELSE N'NONE' END
        ) AS source_scope,
        (SELECT autoID FROM base) AS source_rule_auto
);
GO
...
-- 11.12 กลุ่มส่วนลด (legacy 1.8 ห้ากลุ่ม — ยืนยันชื่อกับศูนย์ก่อนใช้จริง)
INSERT INTO [dbo].[tb_discount_group] ([prefix],[group_code],[group_name],[description],[update_by]) VALUES
 (N'DCG', N'G1', N'กลุ่ม 1', N'legacy : กลุ่มส่วนลดที่ 1', N'System'),
 (N'DCG', N'G2', N'กลุ่ม 2', N'legacy : กลุ่มส่วนลดที่ 2', N'System'),
 (N'DCG', N'G3', N'กลุ่ม 3', N'legacy : กลุ่มส่วนลดที่ 3', N'System'),
 (N'DCG', N'G4', N'กลุ่ม 4', N'legacy : กลุ่มส่วนลดที่ 4', N'System'),
 (N'DCG', N'G5', N'กลุ่ม 5', N'legacy : กลุ่มส่วนลดที่ 5', N'System');
GO
...
-- 11.14 กลุ่มสิทธิ์ (สเปก M002 : privilege group)
INSERT INTO [dbo].[tb_privilege_group] ([prefix],[group_code],[group_name],[module_code],[remark],[update_by]) VALUES
 (N'PVG', N'SYSTEM',   N'ระบบและผู้ใช้งาน', N'M002', N'ผู้ใช้ บทบาท และสิทธิ์', N'System'),
 (N'PVG', N'MASTER',   N'ข้อมูลพื้นฐาน',     N'M003', N'20 resource ของ CRUD engine', N'System'),
 (N'PVG', N'DOCUMENT', N'เอกสาร',           N'M004', N'ใบรับสินค้า ใบส่งหนังสือ ใบรับคืน ใบส่งคืนคู่ค้า', N'System'),
 (N'PVG', N'STOCK',    N'สต็อกและฝากขาย',    N'M005', N'สต็อก ฝากขาย การจัดสรร', N'System');
GO

-- 11.15 บทบาท
--      สเปก M002 หน้า 24 : Role ไม่มีหน้าจอ เพิ่ม/ลบ/แก้ต้องทำที่ฐานโดย database admin
--      is_system = 1 คือสองบทบาทที่ระบบพึ่งพา ลบไม่ได้โดยนิยาม
INSERT INTO [dbo].[tb_role] ([prefix],[role_code],[role_name],[description],[is_system],[update_by]) VALUES
 (N'RLE', N'ADMIN', N'ผู้ดูแลระบบ', N'เทียบเท่า user_level = ADMIN ก่อน v12', 1, N'System'),
 (N'RLE', N'USER',  N'ผู้ใช้งาน',   N'เทียบเท่า user_level = USER ก่อน v12',  1, N'System');
GO

-- 11.16 สิทธิ์ต่อบทบาท
--      ชุดนี้ลอกกฎที่ PenbunAPI บังคับอยู่จริงวันนี้มาทั้งดุ้น ไม่ได้ขยายและไม่ได้ตัด :
--        - ข้อมูลพื้นฐาน  : ทุกคนที่ login อ่านได้ เขียนได้เฉพาะ ADMIN
--                          (crud.Resource.RequireLevel / RequireLevelWrite)
--        - เอกสาร        : งานประจำวัน ทุกคนที่ login ทำได้ครบ
--        - สต็อก/ฝากขาย  : อ่านได้ทุกคน  ปรับ/โอน/rebuild เฉพาะ ADMIN
--        - การจัดสรร     : ดึงประวัติ (pull) เป็นงานประจำวัน จึงให้ insert กับ USER ด้วย
--        - ผู้ใช้งาน     : ADMIN เท่านั้น ทั้งอ่านและเขียน
--      ติดตั้ง v12 แล้วสิทธิ์ของทุกคนจึงเท่าเดิมทุกประการ
...
SELECT N'PRV', r.autoID, g.autoID, v.resource_code,
  ...
  FROM (VALUES
     (N'ADMIN', N'SYSTEM', N'users', 1,1,1,1),
     (N'ADMIN', N'MASTER', N'company', 1,1,1,1),
     (N'ADMIN', N'MASTER', N'customer-type', 1,1,1,1),
     (N'ADMIN', N'MASTER', N'vendor-type', 1,1,1,1),
     (N'ADMIN', N'MASTER', N'discount-type', 1,1,1,1),
     (N'ADMIN', N'MASTER', N'discount-group', 1,1,1,1),
     (N'ADMIN', N'MASTER', N'product-category', 1,1,1,1),
     (N'ADMIN', N'MASTER', N'product-format-type', 1,1,1,1),
     (N'ADMIN', N'MASTER', N'unit-type', 1,1,1,1),
     (N'ADMIN', N'MASTER', N'book-type', 1,1,1,1),
     (N'ADMIN', N'MASTER', N'warehouse', 1,1,1,1),
     (N'ADMIN', N'MASTER', N'product-group', 1,1,1,1),
     (N'ADMIN', N'MASTER', N'vendor', 1,1,1,1),
     (N'ADMIN', N'MASTER', N'customer', 1,1,1,1),
     (N'ADMIN', N'MASTER', N'discount', 1,1,1,1),
     (N'ADMIN', N'MASTER', N'route', 1,1,1,1),
     (N'ADMIN', N'MASTER', N'customer-route', 1,1,1,1),
     (N'ADMIN', N'MASTER', N'product', 1,1,1,1),
     (N'ADMIN', N'MASTER', N'product-sku', 1,1,1,1),
     (N'ADMIN', N'MASTER', N'book', 1,1,1,1),
     (N'ADMIN', N'MASTER', N'price-rule', 1,1,1,1),
     (N'ADMIN', N'DOCUMENT', N'receive-note', 1,1,1,1),
     (N'ADMIN', N'DOCUMENT', N'order', 1,1,1,1),
     (N'ADMIN', N'DOCUMENT', N'return-note', 1,1,1,1),
     (N'ADMIN', N'DOCUMENT', N'vendor-return-note', 1,1,1,1),
     (N'ADMIN', N'STOCK', N'stock', 1,1,1,1),
     (N'ADMIN', N'STOCK', N'consign', 1,1,1,1),
     (N'ADMIN', N'STOCK', N'allocation', 1,1,1,1),
     (N'USER', N'MASTER', N'company', 1,0,0,0),
     (N'USER', N'MASTER', N'customer-type', 1,0,0,0),
     (N'USER', N'MASTER', N'vendor-type', 1,0,0,0),
     (N'USER', N'MASTER', N'discount-type', 1,0,0,0),
     (N'USER', N'MASTER', N'discount-group', 1,0,0,0),
     (N'USER', N'MASTER', N'product-category', 1,0,0,0),
     (N'USER', N'MASTER', N'product-format-type', 1,0,0,0),
     (N'USER', N'MASTER', N'unit-type', 1,0,0,0),
     (N'USER', N'MASTER', N'book-type', 1,0,0,0),
     (N'USER', N'MASTER', N'warehouse', 1,0,0,0),
     (N'USER', N'MASTER', N'product-group', 1,0,0,0),
     (N'USER', N'MASTER', N'vendor', 1,0,0,0),
     (N'USER', N'MASTER', N'customer', 1,0,0,0),
     (N'USER', N'MASTER', N'discount', 1,0,0,0),
     (N'USER', N'MASTER', N'route', 1,0,0,0),
     (N'USER', N'MASTER', N'customer-route', 1,0,0,0),
     (N'USER', N'MASTER', N'product', 1,0,0,0),
     (N'USER', N'MASTER', N'product-sku', 1,0,0,0),
     (N'USER', N'MASTER', N'book', 1,0,0,0),
     (N'USER', N'MASTER', N'price-rule', 1,0,0,0),
     (N'USER', N'DOCUMENT', N'receive-note', 1,1,1,1),
     (N'USER', N'DOCUMENT', N'order', 1,1,1,1),
     (N'USER', N'DOCUMENT', N'return-note', 1,1,1,1),
     (N'USER', N'DOCUMENT', N'vendor-return-note', 1,1,1,1),
     (N'USER', N'STOCK', N'stock', 1,0,0,0),
     (N'USER', N'STOCK', N'consign', 1,0,0,0),
     (N'USER', N'STOCK', N'allocation', 1,1,0,0)
...
-- 11.17 ผูกผู้ใช้ตั้งต้นเข้าบทบาท
--      user_level ยังอยู่และยังเป็นตัวที่ PenbunAPI ใช้จริงใน v12
--      แถวนี้คือสะพาน : พอ API ย้ายไปอ่าน vw_user_privilege แล้วค่อยตัดคอลัมน์ทิ้ง
INSERT INTO [dbo].[tb_user_role] ([prefix],[ref_user_auto],[ref_role_auto],[update_by])
SELECT N'URO', u.autoID, r.autoID, N'System'
  FROM dbo.tb_users u
  CROSS JOIN dbo.tb_role r
 WHERE u.user_name = N'admin' AND r.role_code = N'ADMIN';
GO
...
/* =====================================================================================
   ...
   -------------------------------------------------------------------------------------
   ตัวเลขที่ต้องได้ใน v12 — v11 บวกของ RBAC สี่ตาราง
   !!! ตัวเลขชุดนี้นับจากไฟล์ ยังไม่ได้วัดจากการติดตั้งจริง ต่างจาก v4-v11
       ที่นับจากฐานหลังรันจริง  ติดตั้งครั้งแรกให้รัน SECTION นี้แล้วยืนยันก่อน !!!
     tables 38 · views 36 · procedures 11 · functions 1
     triggers 152 · foreign_keys 59 · check_const 19 · indexes 130
     และ untrusted_fk ต้องเป็นศูนย์เสมอ
   ส่วนต่างจาก v11 : tables +4 · views +3 · triggers +16 · foreign_keys +4 · indexes +8
   ===================================================================================== */
...
UNION ALL SELECT 'functions',   COUNT(*) FROM sys.objects      WHERE name LIKE 'UFN[_]%' AND type IN ('IF','FN','TF')
...
/* =====================================================================================
   ...
   13.5  สมมติฐานที่ตั้งไว้ (ถ้าไม่ตรง แก้ก่อนอย่างอื่น)
         ส่วนลด : เจาะจงกว่าทับกว้างกว่า และ is_on_top บวกเพิ่มบนผู้ชนะ
                   ลำดับเต็มอยู่ในหัวไฟล์ [C] — legacy ไม่ได้เขียนไว้ ต้องให้ศูนย์ยืนยัน
   ...
   13.6  ยังไม่ได้ทำ
         - History Log (tb_history_group / tb_history_log) + tb_configuration
           สเปก M002 บังคับคู่กับ RBAC : ทุก insert/update/delete บนผู้ใช้และสิทธิ์
           ต้องเขียนประวัติ และหน้าจอต้องแสดงย้อนหลัง 5 รายการ
         - Invoice / Credit Note / Vendor Settlement
           ติดคำถามที่ยังค้างข้อ 5 · 6 · 7 ใน SQL-TABLE.md
         - Multi-Company  ติดคำถามข้อ 8 (กทม. 21 / ตจว. 11 คนละนิติบุคคลหรือไม่)

         RBAC ทำแล้วใน v12 — ดูหัวไฟล์ [A]-[E]
         หมายเหตุเดิมของ v8-v11 เขียนว่า "รอคำตอบข้อ 4-14" ซึ่งชี้ไปยังรายการที่
         ไม่มีอยู่ในรีโป  รายการจริงคือ Roadmap 7 ข้อ และคำถามที่ยังค้าง 8 ข้อ
         ใน SQL-TABLE.md  แท็กนั้นจึงถูกตัดออกใน v12
   ===================================================================================== */
