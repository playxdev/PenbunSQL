/* =====================================================================================
   SQL-PENBUN-v7.sql   —   STANDALONE FULL BUILD
   -------------------------------------------------------------------------------------
   Generated : 2026-08-21
   Scope     : DROP & CREATE ทั้งฐานข้อมูล  32 ตาราง  ไม่มี ALTER แม้แต่บรรทัดเดียว
   Replaces  : SQL-PENBUN-v5.sql และ SQL-PENBUN-v6.sql  (ห้ามรันร่วมกัน)

   !!! คำเตือน : SECTION 1 ลบข้อมูลทั้งหมด  สำรองก่อนรันเสมอ !!!

   ---------------------------------------------------------------------------------
   สิ่งที่ v7 แก้จาก v5
   ---------------------------------------------------------------------------------
   [1] FOREIGN KEY 0 -> 51 ตัว  ครบทั้ง 32 ตาราง
       ทุกความสัมพันธ์อ้างด้วย ref_<parent>_auto (INT -> autoID ซึ่งเป็น PK)
       เหตุผล : business id เป็น NULL ตอน INSERT (Trigger เติมทีหลัง) จึงต้องใช้
       filtered unique index ซึ่ง SQL Server ไม่ยอมให้รองรับ FK  -> อ้าง autoID แทน
       ได้ index 4 byte แทน nvarchar(50) 100 byte (เล็กลง ~25 เท่า) และ
       FK แบบ NO ACTION ทำให้ลบตารางแม่ที่มีลูกอ้างอยู่ไม่ได้

   [2] tb_book ไม่ลอยอีกต่อไป
       v5 : tb_book ไม่ผูกกับ tb_product เลย -> ข้อมูลหนังสือซ้ำ 2 ที่
       v7 : tb_book เป็น extension 1:1 ของ tb_product (UQ_tb_book_product)
            และมี ref_book_type_auto, isbn, cover_price, net_price,
            vendor_discount_percent, customer_discount_percent ตามหน้าจอ legacy

   [3] เพิ่ม Stock Ledger
       tb_stock_movement = ความจริง (append-only)
       tb_product_stock / tb_consign_balance = cache สร้างใหม่ได้

   [4] เพิ่ม Route / Consignment / Transaction Layer (14 ตาราง)

   [5] ปิด Hard Delete ที่ระดับ DB
       TRIG_BLOCK_DELETE_* แปลง DELETE เป็น Soft Delete อัตโนมัติทุกตาราง

   [6] ล้างหนี้ทางเทคนิคของ v5
       - ลบ computed column tb_warehouse.location (เคยทำให้ INSERT พัง) -> ใช้ address/province
       - tb_warehouse.warehouse_type ใหม่ (DC/BRANCH/RETURN/DAMAGED/PROVINCE/INTERNATIONAL)
       - tb_users มีฟิลด์ครบตาม Authentication Spec (M001)
       - tb_customer มี is_vat / invoice_format / discount_group / report_name ตาม legacy
       - tb_vendor มี branch_code + ข้อมูลธนาคาร + เงื่อนไขฝากขาย
       - ID trigger เป็น SET-BASED ไม่ใช้ CURSOR
       - CHECK constraint คุมค่าสถานะ
       - trigger บังคับ invariant : is_delete = 1 => is_active = 0 + id_status = 'DELETED'
...
BEGIN
    ...
    DECLARE @nAuto INT, @wh INT, @ven INT, @status NVARCHAR(20),
            @docDate DATETIME, @docNo NVARCHAR(30);
...
BEGIN
    ...
    DECLARE @oAuto INT, @wh INT, @cus INT, @route INT,
            @period NVARCHAR(20), @status NVARCHAR(20), @docDate DATETIME, @docNo NVARCHAR(30);
...
BEGIN
    ...
    DECLARE @rAuto INT, @cus INT, @period NVARCHAR(20),
            @status NVARCHAR(20), @docDate DATETIME, @docNo NVARCHAR(30);
    ...
    BEGIN TRAN;
        ...
        UPDATE dbo.tb_return_note SET doc_status = N'POSTED', update_by = @UpdateBy WHERE autoID = @rAuto;
    COMMIT TRAN;
END
GO
...
BEGIN
    ...
    BEGIN TRAN;
        ...
        UPDATE dbo.tb_vendor_return_note
           SET doc_status = N'POSTED', update_by = @UpdateBy WHERE autoID = @nAuto;
    COMMIT TRAN;
END
GO
